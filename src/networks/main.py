...
def build_network(net_name, ae_net=None):
    ...
    if net_name == 'titansoft_mlp':
        net = MLP(x_dim=17, h_dims=[32, 16], rep_dim=8, bias=False)
...
def build_autoencoder(net_name):
    ...
    if net_name == 'titansoft_mlp':
        ae_net = MLP_Autoencoder(x_dim=17, h_dims=[32, 16], rep_dim=8, bias=False)
